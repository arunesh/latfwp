...
Trust and Stake are combined into a vector quantity called the {\em l-factor}. The l-factor is computed as follows:

\newcommand\LFACTOR{\overrightarrow{\mathit{\text{l-factor}}}}
\newcommand\SIG{\mathit{\text{SIG}}}
\begin{equation*}
    \label{eq_l_factor}
    \LFACTOR(E) = \eta \SIG(S(E)\hat{k} + \gamma \SIG(T(E))\hat{j}
\end{equation*}
where
\begin{itemize}
    \item $\SIG(X)$ is the Sigmoid function. \[ \SIG(X) = \frac{1}{1 + e^{-x}}\]
    \item $|\LFACTOR(E)|$: The value (norm) of the l-factor metric for participant E and $\LFACTOR(E)$ is the vector.
    ...
    \item $\hat{k}$ and $\hat{j}$ are orthogonal unit vectors.
\end{itemize}

The l-factor value combines both trust and stake into a vector metric which is used to determine if a node can be
allowed to participate in key network operations. The vector metric allows conditions to be placed on both dimensions of
trust and stake independently. These operations include block production, acting as delegates, or
...
%new spatial data types, indexes, cryptographic proofs and other mechanisms that are fundamental to the platform.
